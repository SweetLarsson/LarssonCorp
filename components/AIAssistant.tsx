...
const AIAssistant: React.FC<AIAssistantProps> = ({ onClose, theme = 'dark' }) => {
  ...
  const formatText = (text: string) => {
    ...
    const flushTable = (key: number) => {
      ...
      tableRows = [];
      return (
        <div key={`table-${key}`} className="my-6 overflow-x-auto rounded-xl border border-larsson-accent/20">
    ...
    lines.forEach((line, index) => {
      ...
      if (trimmed === '---') {
        ...
        elements.push(<hr key={index} className={`my-6 border-t ${isDark ? 'border-white/10' : 'border-black/10'}`} />);
        return;
      }
      ...
      if (trimmed.startsWith('* ') || trimmed.startsWith('- ')) {
        ...
        elements.push(
          <div key={index} className="flex gap-3 ml-3 mb-2">
            <span className="text-larsson-accent font-black select-none">•</span>
            <p className="text-[11px] font-medium leading-relaxed">{bulletText.replace(/\*\*/g, '')}</p>
          </div>
        );
        return;
      }
      ...
      if (trimmed) {
        elements.push(
          <p key={index} className="mb-3 text-[11px] font-medium leading-relaxed opacity-90">
            {trimmed.replace(/\*\*/g, '')}
          </p>
        );
      } else {
  ...
  return (
    <div 
      ...
      className={`fixed right-6 md:right-10 w-[85vw] md:w-[35vw] max-w-[500px] border shadow-3xl flex flex-col overflow-hidden animate-slide-up z-[80] rounded-[2.5rem] backdrop-blur-3xl transition-all duration-500 ${isDark ? 'bg-larsson-black/90 border-white/10' : 'bg-white/90 border-black/10'}`}
    >
      {/* Header */}
      <div className={`px-8 py-5 flex items-center justify-between border-b shrink-0 ${isDark ? 'border-white/5' : 'border-black/5'}`}>
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-larsson-accent flex items-center justify-center font-black text-white text-[12px] shadow-lg">L</div>
          <div className="flex flex-col">
            ...
            <span className="text-larsson-accent font-black text-[8px] uppercase tracking-tighter">Strategic Insights</span>
          </div>
        </div>
      ...
      <div ref={scrollRef} className="flex-1 p-8 overflow-y-auto space-y-6 scrollbar-hide">
        {messages.map((m, i) => (
          <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div className={`relative group max-w-[95%] p-6 rounded-3xl transition-all ${
              m.role === 'user' 
                ? 'bg-larsson-accent text-white rounded-br-none font-bold shadow-lg' 
        ...
        {isLoading && <div className="text-[9px] font-black text-larsson-accent uppercase tracking-[0.4em] animate-pulse ml-2">Orchestrating...</div>}
      </div>
      ...
      <div className={`p-8 border-t shrink-0 ${isDark ? 'border-white/5 bg-larsson-black/20' : 'border-black/5 bg-black/[0.02]'}`}>
        <div className="flex gap-3">
          <input
            ...
            className={`flex-1 px-6 py-4 text-[12px] font-bold rounded-2xl outline-none focus:ring-2 focus:ring-larsson-accent/30 transition-all ${isDark ? 'bg-white/5 text-white border-white/10' : 'bg-black/5 text-larsson-black border-black/10'}`}
          />
          <button onClick={handleSend} className="bg-larsson-accent text-white p-4 rounded-2xl hover:scale-105 active:scale-95 transition-all flex items-center justify-center shrink-0 shadow-lg shadow-larsson-accent/20">
